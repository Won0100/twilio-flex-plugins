...
class SecurityQuestions extends React.Component {
  ...
  getSecQuestions() {
    ...
    if (
      ...
      const crmProxyUrl = 'https://test-service-2641.twil.io/crm-proxy'
      fetch(/* insert URL of the proxy function + profile ID here */ `${crmProxyUrl}?id=${this.props.task.attributes.account_number}`)
        .then((response) => {
          if (response.ok) {
